...
class Scrape_Saver:
    ...
    def retrieve(self, item):
        if (os.path.isfile("../test_files/" + item + ".fasta")):
            with open('../test_files/' + item + '.fasta', 'r') as infile:
                print("reading form local File")
                return infile.read()
        else :
            print("reading form URl")
            f = open('../test_files/' + item + '.fasta', 'w+')
            url = "http://www.uniprot.org/uniprot/{0}.fasta".format(item)
